...
public class SATFileReader {
	...
	// Main mehtod
	public static int[][] getFormula(String fileName)
	{
		filename = fileName;
		openSATFile();
	...
	// ****************************************************************************************************
	// Make sure the file exisits and if so prepare to read it in, if not loop
	// till a valid file is given
	...
	//fitness function
	public static int evalSAT(boolean[] bitstring)
	{
		int fitness = 0;
		
		for(int[] clause : SATFormula)
			if(evalClause(clause, bitstring))
				fitness++;
		
		return fitness;
	}
	
	// evaluates a single SAT clause
	private static boolean evalClause(int[] clause, boolean[] bitstring)
	{	
		for(int i = 0; i < clause.length; ++i)
		{
			int index = clause[i];
			
			//to account for clauses shorter than maxClause length
			if(index == 0)
				break;
...
*/
			...
			try {
				...
				System.out.println("Please specify a valid SAT plan File: "
						+ filename);
				filename = readString(System.in);
				openSATFile();
			}
